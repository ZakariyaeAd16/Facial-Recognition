...
mydb = mysql.connector.connect(
    ...
    user="root",
    passwd="",
    database="gestionpresence"
)
mycursor = mydb.cursor()


#  Generation du dataset(Capture les images du visage et les enregistre dans la DB)
def generate_dataset(nbr):
    face_classifier = cv2.CascadeClassifier(
        "C:/Users/Lenovo/Desktop/gestionPresence/facialRecognition/resources/haarcascade_frontalface_default.xml")

    def face_cropped(img):

        gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)  #Convertir les images en gris pour ameliorer la detection
        faces = face_classifier.detectMultiScale(gray, 1.3, 5)  #Detection des visages
...
def train_classifier(nbr):
    dataset_dir = "C:/Users/Lenovo/Desktop/gestionPresence/facialRecognition/dataset"
        ...
        for (x, y, w, h) in features:
            ...
            if confidence > 70 and not justscanned:
                ...
                if int(cnt) == 30:
                    ...
                    mycursor.execute("insert into presence_historique (accs_date, accs_prsn) values('" + str(
                        date.today()) + "', '" + pnbr + "')")
                    mydb.commit()
...
def addprsn():
    ...
    return render_template('addprsn.html', student_data=student_data)
